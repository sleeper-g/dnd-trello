...
export default class Board {
  ...
  showPossiblePlace(event) {
    ...
    if (event.target.classList.contains("tasks-list")) {
      const listCard = [...event.target.children].filter(
        (card) => 
      ...
      if (listCard && !listCard.length){
        ...
        if (event.clientY - 146 > listCard[listCard.length - 1].offsetTop 
          ...
          for (let item in listCard){
            let res = ((event.clientY - 100 > listCard[item].offsetTop)
                && (event.clientY - 100 < listCard[item].offsetTop + listCard[item].offsetHeight / 2)) ? true : false;
            if (res) {
            this.hidden.remove();
